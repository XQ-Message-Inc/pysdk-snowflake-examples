...
# Encrypt a field using the AES256 algorithm
def encrypt_field_aes(field_data, key_packet, locator_token):
    try:
        encrypted_text, nonce, tag = xq.encrypt_message(field_data, key=key_packet, algorithm="AES")
        return locator_token + base64.b64encode(nonce).decode() + base64.b64encode(encrypted_text).decode()
...
def encrypt_row(row, key_packet, xq):
    ...
    for col in row.index:
        ...
        if isinstance(row[col], str):  # Check if the value is a string to encrypt
            row[col] = encrypt_field_aes(field_data=row[col], key_packet=shuffled_key, locator_token=locator_token)
    return row
