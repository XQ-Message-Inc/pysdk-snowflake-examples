...
def decrypt_field_otp(field_data, locator_token):
    key_packet = xq.api.get_packet(locator_token)
    try:
        decrypted_text = xq.decrypt_message(base64.b64decode(field_data), key_packet, algorithm="OTP")
...
if __name__ == "__main__":
    ...
    for row in pdf.itertuples(index=True):
        decrypted_values = [decrypt_field_otp(getattr(row, col)[43:], getattr(row, col)[0:43]) for col in pdf.columns if col != 'ID']
